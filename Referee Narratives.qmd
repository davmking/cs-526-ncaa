...
foul_disparity_games <- foul_disparity_games %>% 
  left_join(pbp_data_long, by = "game_id")

# Mostly pretty low, but there are some kinda crazy disparities
...
foul_disparity_games %>% 
  filter(game_id == 401408616) %>% 
  arrange(play_id) %>% 
  select(play_id, half, time_remaining_half, action_team, description) %>% 
  print(n = 346)
```
